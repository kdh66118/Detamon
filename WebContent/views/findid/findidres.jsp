			...
			<form action="" method="">
				...
				<c:choose>
					<c:when test="${findid == null || findid == '' }">
						...
						<button type="button" class="find_id_resbtn" onclick="location.href='controller.do?command=findidform'">아이디 찾기</button>
					</c:when>
					<c:otherwise>
						<ul class="search__info-list">
							<li class="search__info-item">
						...
						<button type="button" class="find_id_resbtn" onclick="location.href='controller.do?command=home'">홈</button>
						<button type="button" class="find_id_resbtn" onclick="location.href='controller.do?command=findpwform&id=${findid}'">비밀번호 찾기</button>
					</c:otherwise>
			</c:choose>				
